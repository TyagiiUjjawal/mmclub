...
const saveUtrController = async (req, res) => {
    ...
    try {
        // Query to find the user's phone number by the token
        ...
        // Update the record with the provided UTR number where status is 'Pending'
        const sql = `
            UPDATE rechargeRequests 
            SET utrNumber = ? 
            WHERE mobileNumber = ? AND status = 'Pending'
            LIMIT 1
        `;

        const [result] = await connection.query(sql, [utrNumber, mobileNumber]);
        if (result.affectedRows === 0) {
            return res.status(404).send('No pending payment found for this user.');
        }

        res.status(200).send('UTR number updated successfully!');
    } catch (err) {
        console.error('Error updating UTR number:', err);
...
const withdrawal3 = async (req, res) => {
    ...
    if (!auth || !money || !password || money < 299) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
            timeStamp: new Date().toISOString(), // Using ISO format for timestamp
        });
    }

    // Fetch user data
    const [user] = await connection.query('SELECT `phone`, `code`, `invite`, `money`, `betSurplus` FROM users WHERE `token` = ? AND password = ?', [auth, md5(password)]);
    ...
    function timerJoin(params = '', addHours = 0) {
        ...
        let ampm = date.getHours() < 12 ? 'AM' : 'PM';
        let minutes = formatT(date.getMinutes());
        let seconds = formatT(date.getSeconds());
    ...
    let total = recharge.reduce((sum, data) => sum + parseFloat(data.money), 0);
    let total2 = minutes_1.reduce((sum, data) => sum + parseFloat(data.money), 0);
    ...
    if (user_bank.length !== 0) {
        if (withdraw.length < 3) {
            if (userInfo.money - money >= 0) {
                if (result < money) {
                    return res.status(200).json({
                        message: 'You need to bet up to your balance to fulfill the request.',
                        status: false,
                        timeStamp: new Date().toISOString(),
                    });
                }
